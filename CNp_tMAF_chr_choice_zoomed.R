...
for (i in c(17)) {
	...
	maxPosition <- 20000000
	
	png(filename = paste("Patient1_CNp_tLOH_chr", i, "_zoomed.png", sep = ""), h=1200, w=1800, pointsize=20)
	plot(1:10)
	layout(matrix(c(1, 2), 2, 1, byrow = TRUE))
	par(mar = c(0, 0, 0, 0), oma = c(5, 4.5, 5, 2.25) + 0.1, xpd = NA)
	...
	if (length(tt)>0) {
        ...
        abline(v=7577068, col='red', lwd=3)
        par(xpd=NA)
